...
while email != "End":
    ...
    if "@" not in email:    # elif
        raise MustContainAtSymbolError("Email must contain @!")
    if len(email.split("@")[0]) <= MIN_NAME_SYMBOLS_COUNT:   # elif
        raise NameTooShortError("Name must be more than 4 characters!")
    if email.split(".")[-1] not in VALID_DOMAINS:   # elif
        raise InvalidDomainError(f"Domain must be one of the following: {', '.join('.' + d for d in VALID_DOMAINS)}")
    if findall(pattern_name, email.split("@")[0])[0] != email.split("@")[0]:     # elif
