...
while tea_bags < 10:  # развъртаме цикъл, докато Алиса не събере 10 пакетчета с чай
    ...
    element = matrix[row][col]  # запазваме стойността на позицията на Алиса
    matrix[row][col] = '*'  # променяме стойността на позицията й на звезда

    if element == 'R':  # проверяваме дали Алиса не е попаднала в заешката дупка
        break  # прекратяваме цикъла

    if element.isdigit():  # проверяваме дали Алиса е стъпила на пакетче с чай
        tea_bags += int(element)  # увеличаваме броя на пакетчетата, които Алиса е събрала
